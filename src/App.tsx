  ...
  return (
    <OuterWrapper divRef={divRef} width={width}>
      <MainWrapper>
        ...
        <ContainerListFilter>
          <DragDropContext onDragEnd={(result) => dispatch({type: TodoKind.DRAG, payload: {e: result, filteredTodos: filteredTodos}})}>
            <List dispatch={dispatch} filteredTodos={filteredTodos}/> 
          </DragDropContext>
          <Filter setFilterStatus={setFilterStatus} Refs={{allRef, activeRef, completeRef}} dispatch={dispatch} activeItems={currentItemCount(state)} width={width}/>
        </ContainerListFilter>
        <DragAndDropText />
      </MainWrapper>
    </OuterWrapper>
