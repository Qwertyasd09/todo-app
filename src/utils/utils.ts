...
export const getFilteredTodos = (filterStatus: "all" | "active" | "complete", state: TodoItem[]): TodoItem[] => {
  let filteredTodos: TodoItem[] = []
  if (filterStatus === "all") {
    filteredTodos = state.map((todo) => {return {...todo}})
  } else if (filterStatus === "active") {
    filteredTodos = state.map(todo => {return {...todo}}).filter((todo) => todo.done !== true)
...
export const getRef = (option: string, {allRef, activeRef, completeRef}: Refs) => {
  switch(option) {
    case "All":
      return allRef;
    case "Active":
...
export const handleFilterSelect = (e: React.MouseEvent<HTMLLIElement>, {Refs, setFilterStatus}: FilterSelect) => {
...
export const reducer = (state: TodoItem[], action: TodoActions) => {
  switch (action.type) {
    ...
    case TodoKind.CHECK:
      const checkedTodo = (action.payload.target as HTMLDivElement | SVGSVGElement).getAttribute("id");
